class SchedulingPolicy < ApplicationPolicy
  ...
  def update?
    return true if permissions.include?('schedule_update') && (user.role_name=='bcba' || user.role_name=='administrator' || user.role_name=='executive_director' || user.role_name=='super_admin')
