      ...
      def seed_staff_data(username, password)
        ...
        staff_rosters.each do |staff_roster|
          ...
          if Staff.where(email: staff_roster['email']).blank?
            staff = Staff.find_or_initialize_by(email: staff_roster['email'], dob: staff_roster['dob']&.to_time&.strftime('%Y-%m-%d'), hired_on: staff_roster['hireddate']&.to_time&.strftime('%Y-%m-%d'), first_name: staff_roster['stafffirstname'], last_name: staff_roster['stafflastname'], terminated_on: staff_roster['terminateddate']&.to_time&.strftime('%Y-%m-%d'))
