    ...
    context "when sign in" do
      it "should create client note successfully" do
        ...
        post :create, params: {
          client_id: client.id,
          note: 'test-note',
          attachment_attributes: { base64: 'data:image/gif;base64,R0lGODdhAQABAPAAAP8AAAAAACwAAAAAAQABAAACAkQBADs='}
        }
        response_body = JSON.parse(response.body)
        ...
        expect(response_body['data']['attachment']).not_to eq(nil)
      end
    end
  end
  ...
  describe "PUT #update" do
    context "when sign in" do
      ...
      context "and update associated data" do
        it "should update associated attachment successfully" do
          set_auth_headers(auth_headers)
