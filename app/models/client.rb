class Client < ApplicationRecord
  ...
  scope :by_payor, ->(payor_name){ joins(client_enrollments: :funding_source).where("client_enrollments.is_primary": true).where("funding_sources.name ILIKE '%#{payor_name}%'") }
